...
module lemmas-progress-checks where
  ...
  indet-not-trans (IFst (IPair1 ind x) h) ITFst = h refl
  indet-not-trans (IFst (IPair2 x ind) h) ITFst = h refl
  indet-not-trans (ISnd (IPair1 ind x) h) ITSnd = h refl
  indet-not-trans (ISnd (IPair2 x ind) h) ITSnd = h refl
  indet-not-trans (IPair1 ind x) ()
  indet-not-trans (IPair2 x ind) ()
  indet-not-trans (ICastProd x ind x₁) ITCastID = x refl
  indet-not-trans (ICastProd x ind x₁) (ITPairCast x₂) = x₁ refl
  ...
  final-sub-final (FIndet (IFst x₁ _)) (FHFst eps) = final-sub-final (FIndet x₁) eps
  final-sub-final (FIndet (ISnd x₁ _)) (FHSnd eps) = final-sub-final (FIndet x₁) eps
  ...
  final-sub-final (FIndet (ICastProd x₁ x₂ x₃)) (FHCast eps) = final-sub-final (FIndet x₂) eps
